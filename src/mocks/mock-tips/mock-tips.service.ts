...
export class MockTipsService {
  private readonly mockTips = [
    ...
    },
    {
      ...
      body: "Always use 'use strict' aiujjhjt the beginning of your JavaScript files.",
  ...
  async getTips(level: string, technology: string): Promise<any> {
    const tips = this.mockTips.filter(
      (tip) =>
        tip.level.some((s) => s.name === level) &&
        tip.technology.some((t) => t.name === technology),
    );

    if (tips.length === 0) {
      throw new NotFoundException(
        `No tips found for seniority '${level}' and technology '${technology}'`,
      );
    }
